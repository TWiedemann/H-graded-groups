...
# Here R denotes the set of simple reflections in Weyl(H4). Claimed in [BW, 4.8].
...
# I.e. test that there exists rho with certain properties.
...
# Returns true if H4ParitySimRoot(alpha, i) = H4ParitySimRoot(-alpha, i) for all alpha in H4 and i in [1..4]. Claimed in [BW, 4.27, 6.16].
...
# I.e. the output is precisely [BW, Figures 12, 13] and the function verifies [BW, (4.28)] in [BW, 4.27].
